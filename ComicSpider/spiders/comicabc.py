...
from utils.website.providers.comicabc import ComicabcUtils
from .basecomicspider import BaseComicSpider, ComicspiderItem


class ComicabcSpider(BaseComicSpider):
    name = "comicabc"
    image_ua = ComicabcUtils.image_ua
    custom_settings = {
    ...
    def _process_episode(self, ep):
        ...
        if not page_urls or not chapter_referer:
            ...
            raise ValueError(f"comicabc episode requires {missing}: {ep!r}")
