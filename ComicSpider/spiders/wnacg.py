# -*- coding: utf-8 -*-
import re
from .basecomicspider import BaseComicSpider2, font_color, ComicspiderItem
...
class WnacgSpider(BaseComicSpider2):
    ...
    def frame_section(self, response):
        # book_index = self.index_regex.search(response.url).group(1)
        # to_book_index = self.rule_book_index(book_index)
        doc_wlns = re.split(r';[\n\s]+?document\.writeln', response.text)
        selected_doc = next(filter(lambda x: "var imglist" in x, doc_wlns))
        targets = re.findall(r'(//.*?(jp[e]?g|png))', selected_doc)
