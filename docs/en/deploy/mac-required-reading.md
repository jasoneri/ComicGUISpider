...
Portable `CGS.app` runs `deploy/launcher/mac/init.bash` (via gitee raw) for `brew` + `uv`. scriptWin **aria2c** is pulled on first use from `releases/download/preset` into the user configuration directory (`conf_dir/cgs-aria2/bin`; not brew PATH).
