...
参考 [`jm`](../../ComicSpider/spiders/jm.py) 或 [`wnacg`](../../ComicSpider/spiders/wnacg.py) ，以 wnacg 为例

### WnacgSpider
##### 类属性

- [x] name: 爬虫名字，取目标网站域名的部分或标题，与分支名相同  
- [x] domain: 目标网站域名  
- [x] search_url_head: 搜索页url(去掉关键词)，大部分网站都是以get形式直出的  
- [ ] custom_settings: `scrapy`客制设定。举例两个应用  
...
- [x] frame_section:  
    + 二级页面：书页面 > 能直接获取该书的全页  
    + 三级页面：书页面 > 章节列表 之间的清洗  
- [ ] parse_fin_page: (二级页面不需要，三级页面必须) 章节页面 > 直接获取该章节的全页  
- [ ] mk_page_tasks: 跟四级页面相关，可以用巧妙方法绕过，初始先不管，三级页面情况下参考`kaobei.py`
...
#### [`GUI/mainwindow.py`](../../GUI/mainwindow.py)
