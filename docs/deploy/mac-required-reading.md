...
|   步骤    | 解析说明 |
|:------:|:-----------------------------------|
| 1. 初始化   | 解压后初次运行`CGS.app`时（点击app前阅读上方签名收费的提示）<br>检测没 `brew`、`uv` 会安装，再 `uv tool install` CGS；script 下载引擎 aria2 首次使用时从 `preset` 拉到本地（**macos-arm**） |
