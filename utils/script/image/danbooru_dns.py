...
DANBOORU_DNS_STUB_HOST = "127.0.0.1"
DANBOORU_DNS_STUB_PORT = 53
DANBOORU_WEBENGINE_PROXY_HOST = "127.0.0.1"
DEFAULT_DANBOORU_DOH_URL = "https://cloudflare-dns.com/dns-query"
...
def normalize_doh_url(value: object) -> str:
    ...
    if parsed.scheme != "https" or not parsed.netloc:
        raise ValueError("Danbooru DoH URL must be a full https://... endpoint")
    return text
...
def create_async_doh_resolver(doh_url: str) -> dns.asyncresolver.Resolver:
    ...
    if not normalized_doh_url:
        raise ValueError("Danbooru DoH resolver can only be created when DoH is enabled")
...
async def resolve_host_via_doh(
    ...
    *,
    ...
    if not text:
        raise httpcore.ConnectError("Danbooru DoH resolution received an empty host")
    try:
        return str(ipaddress.ip_address(text))
    except ValueError:
        pass
    if text.casefold() == "localhost":
        return DANBOORU_DNS_STUB_HOST
    ...
    if last_error is not None:
        raise httpcore.ConnectError(f"Danbooru DoH resolution failed for {text}: {last_error}") from last_error
    raise httpcore.ConnectError(f"Danbooru DoH resolution returned no A/AAAA record for {text}")


class _DanbooruDnsProtocol(asyncio.DatagramProtocol):
    ...
    async def _handle_query(self, data: bytes, addr):
        ...
        try:
            ...
            logger.warning(f"[DanbooruDNS] query failed qname={question.name} rdtype={dns.rdatatype.to_text(question.rdtype)} error={exc}")
...
class _DanbooruDnsStubService:
    ...
    def ensure_started(self, doh_url: str, *, timeout: float = 5.0) -> None:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(
                ...
                name="DanbooruDnsStub",
                daemon=True,
            )
            self._thread.start()
        if not self._ready.wait(timeout):
            raise TimeoutError("Danbooru DNS stub startup timed out")
        if self._startup_error is not None:
            raise RuntimeError(
                f"Danbooru DNS stub failed at {DANBOORU_DNS_STUB_HOST}:{DANBOORU_DNS_STUB_PORT}"
    ...
    def _thread_main(self, doh_url: str, ready_event: threading.Event) -> None:
        ...
        try:
            protocol_factory = lambda: _DanbooruDnsProtocol(doh_url)
            transport, _protocol = loop.run_until_complete(
                loop.create_datagram_endpoint(
                    protocol_factory,
                    local_addr=(DANBOORU_DNS_STUB_HOST, DANBOORU_DNS_STUB_PORT),
                )
            )
            with self._lock:
                self._loop = loop
            ...
            logger.info(
                f"[DanbooruDNS] started stub={DANBOORU_DNS_STUB_HOST}:{DANBOORU_DNS_STUB_PORT} doh={doh_url}"
            )
            ready_event.set()
...
_danbooru_dns_stub_service = _DanbooruDnsStubService()


def ensure_danbooru_dns_stub_started(doh_url: str, *, timeout: float = 5.0) -> None:
    _danbooru_dns_stub_service.ensure_started(doh_url, timeout=timeout)


async def ensure_danbooru_dns_stub_started_async(doh_url: str, *, timeout: float = 5.0) -> None:
    await asyncio.to_thread(ensure_danbooru_dns_stub_started, doh_url, timeout=timeout)


def shutdown_danbooru_dns_stub() -> None:
    _danbooru_dns_stub_service.stop()


class _DanbooruWebEngineProxyService:
    def __init__(self):
        self._lock = threading.Lock()
        self._thread: Optional[threading.Thread] = None
        self._ready = threading.Event()
    ...
    def ensure_started(self, doh_url: str, *, timeout: float = 5.0) -> str:
        ...
        with self._lock:
            if self._is_running_locked(normalized_url):
                return f"{DANBOORU_WEBENGINE_PROXY_HOST}:{self._proxy_port}"
            self._stop_locked()
            self._ready = threading.Event()
            self._startup_error = None
            self._doh_url = normalized_url
            ...
            self._thread = threading.Thread(
                target=self._thread_main,
                args=(normalized_url, self._ready),
                name="DanbooruWebEngineProxy",
                daemon=True,
            )
            self._thread.start()
        if not self._ready.wait(timeout):
            raise TimeoutError("Danbooru WebEngine proxy startup timed out")
        if self._startup_error is not None:
            raise RuntimeError("Danbooru WebEngine proxy failed to start") from self._startup_error
        return f"{DANBOORU_WEBENGINE_PROXY_HOST}:{self._proxy_port}"
        ...
        try:
            ...
            server = loop.run_until_complete(
                asyncio.start_server(
                    ...
                    DANBOORU_WEBENGINE_PROXY_HOST,
                    0,
                )
            )
            sockets = server.sockets or []
            if not sockets:
                raise RuntimeError("Danbooru WebEngine proxy did not bind any sockets")
            ...
            with self._lock:
                ...
                self._stop_future = stop_future
            logger.info(
                f"[DanbooruDNS] started webengine proxy={DANBOORU_WEBENGINE_PROXY_HOST}:{self._proxy_port} doh={doh_url}"
            )
            ready_event.set()
            loop.run_until_complete(stop_future)
        except Exception as exc:
    ...
    async def _handle_client(
        ...
        try:
            ...
            logger.warning(f"[DanbooruDNS] webengine proxy request failed error={exc}")
            with suppress(Exception):
...
_danbooru_webengine_proxy_service = _DanbooruWebEngineProxyService()


def ensure_danbooru_webengine_proxy_started(doh_url: str, *, timeout: float = 5.0) -> str:
    return _danbooru_webengine_proxy_service.ensure_started(doh_url, timeout=timeout)


def shutdown_danbooru_webengine_proxy() -> None:
    _danbooru_webengine_proxy_service.stop()


class DanbooruDoHNetworkBackend(httpcore.AsyncNetworkBackend):
    def __init__(self, doh_url: str):
    ...
    async def connect_tcp(
        ...
        await ensure_danbooru_dns_stub_started_async(self._doh_url)
...
class DanbooruAsyncHTTPTransport(httpx.AsyncBaseTransport):
    def __init__(
        self,
        *,
        ...
        if proxy is None:
            ...
            raise ValueError("Danbooru transport only supports direct or HTTP/HTTPS proxy connections")
...
def build_danbooru_async_transport(
    proxy_policy: str,
    proxies: list[str],
    *,
    doh_url: str,
    ...
    if proxy_policy != "direct" and proxies:
        ...
        effective_retries += 1
    transport = DanbooruAsyncHTTPTransport(
        verify=verify,
        trust_env=False,
        proxy=proxy,
        retries=effective_retries,
        network_backend=DanbooruDoHNetworkBackend(normalized_doh_url),
    )
