...
> 如有疑问或建议 前往[🔗去重逻辑说明以及讨论](https://github.com/jasoneri/ComicGUISpider/discussions/23)

✅ 新增`增加标识`开关勾选，为储存目录最后加上网站url上的作品id，专门处理同名不同馅的作品  
✅ 处理拷贝的隐藏漫画  
✅ 细化任务，主进度条启动后，可打开预览窗口在其页面的最上方有`各子任务进度`视图  
✅ 去重记录md5相关：细化任务后，节点变更为在该任务全部图片下载成功时记录

#### 🐞 Fix/修复

✅ 优化ex绅士获取cookies的指引，并处理此前`全局代理对于程序不起作用`的情况  
✅ 优化报错/正常操作而没结果的提示分类  
